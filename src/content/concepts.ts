...
const CONCEPTS: Concept[] = [
  ...
  { id: "config-mirror", term: "the config mirror", layer: "config", introducedIn: "stage-2",
    short: "Keep ONE canonical config and project it to the other engine instead of hand-maintaining two: AGENTS.md is generated from — or symlinked to — CLAUDE.md, and a drift check fails CI if they diverge. That is what makes 'configure once' actually span Claude Code and Codex, so you can switch between them (e.g. in your VS Code editor) without re-teaching each one.",
    example: "A render step writes AGENTS.md from CLAUDE.md; a checker verifies they resolve to the same content, so Codex and Claude read identical rules and you can swap tools mid-task.",

...
const CONCEPTS: Concept[] = [
  ...
  { id: "config-mirror", term: "the config mirror", layer: "config", introducedIn: "stage-2",
    short: "Keep ONE canonical config and project it to the other engine instead of hand-maintaining two. Claude Code reads only CLAUDE.md (never AGENTS.md), so the portable bridge is a first-line `@AGENTS.md` import inside CLAUDE.md (works cross-platform, no admin) — preferred over an `ln -s AGENTS.md CLAUDE.md` symlink (Windows needs admin/Developer Mode). A drift check fails CI if the two diverge, so you can switch between Claude Code and Codex (e.g. in VS Code) without re-teaching each one.",
    example: "CLAUDE.md's first line is `@AGENTS.md`; both engines read the same instructions, and a checker verifies they stay in sync.",
    prerequisites: ["claude-md", "agents-md"], contrasts: [] },
  ...
  { id: "seats", term: "seats & plan gating", layer: "governance", introducedIn: "stage-5",
    ...
    prerequisites: ["claude-code"], contrasts: [] },
  { id: "permission-rule", term: "a permission rule", layer: "governance", introducedIn: "stage-5",
    short: "The actual access-control primitive: allow / ask / deny lists of tool-name glob patterns (e.g. Bash(git push:*), Read(./.env.*), mcp__*) in settings.json. Rules evaluate deny → ask → allow, first-match-wins, and the arrays MERGE across scopes — so a higher-scope (managed or user) deny is a floor no lower scope can relax. This is the mechanism behind 'deny secret reads'.",
    example: "A managed deny rule Read(./.env.*) blocks every teammate from reading secrets, and no project setting can re-allow it.",
    prerequisites: ["settings"], contrasts: ["permission-mode"] },
  { id: "permission-mode", term: "permission mode", layer: "governance", introducedIn: "stage-5",
    short: "An autonomy dial orthogonal to the rules — how aggressively Claude acts, not what is allowed: default (ask), plan (read-only, proposes changes), acceptEdits (auto-approve edits), auto (a classifier model approves/denies each call), and bypassPermissions (no prompts at all). Set via defaultMode; cycle in-session with Shift+Tab.",
    example: "Reviewers run in plan mode (read-only); a trusted batch job runs acceptEdits; bypassPermissions is for sandboxes only.",
    prerequisites: ["claude-code"], contrasts: ["permission-rule"] },
...
export const PREREQ_WHY: Record<string, string> = {
  ...
  "config-mirror>claude-md": "CLAUDE.md is the single canonical source the mirror projects from",
  "config-mirror>agents-md": "the mirror's whole job is to keep AGENTS.md in sync with the canonical source",
  ...
  "permission-rule>settings": "permission rules are the allow/ask/deny lists configured in settings",
  "permission-mode>claude-code": "a permission mode is a runtime autonomy dial of Claude Code",
...
export const PREREQ_KIND: Record<string, PrereqKind> = {
  ...
  "config-mirror>claude-md": "defined-via",
  "config-mirror>agents-md": "operates-on",
  ...
  "permission-rule>settings": "defined-via",
  "permission-mode>claude-code": "assumes",
